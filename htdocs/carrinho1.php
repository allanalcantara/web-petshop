				...
				<tbody>
				  <?php foreach($resultsCarts as $result) : ?>
					<tr>
						...
						<td>
                                                    <input type="text" name="prod[<?php echo $result['idproduto']?>]" value="<?php echo $result['quantidade']?>" size="1" />
                                                </td>
						<td>R$<?php echo number_format($result['preco'], 2, ',', '.')?></td>
						<td>R$<?php echo number_format($result['subtotal'], 2, ',', '.')?></td>
						<td><a href="carrinho1.php?acao=del&id=<?php echo $result['idproduto']?>" class="btn btn-danger">Remover</a></td>
